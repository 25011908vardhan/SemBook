...
<body>
    <div class="bgImg">
    <img src="bg.png" alt="Bg Image" id="bg">
    </div>
    <div class="msg">
      Signup!
    </div>
...
else
{
    if($pass==$cpass)
    {
        $insertquery="insert into registration(name,email,password) values('$name','$email','$enpass')";
        ...
        if($insertqueryExecution)
        {
            ...
            window.location="http://localhost/Hackathon/login.php"
        </script>
            
            <?php
            // $makeTableQry="create table `$email` (id INT(6) unsigned auto_increment primary key  "
            $insertqueryinBooks="insert into books (email) values('$email')";
            $insertqueryinBooksExecution=mysqli_query($con,$insertqueryinBooks);
            if($insertqueryinBooksExecution)
            {
                ?>
                <script>alert("Made 8 books!");</script>
                <?php
            }
            else{
                ?>
                <script>alert("something went wrong while making books");</script>
                <?php
            }
        }
        else{
